...
from pyfibre.shg_pl_trans.shg_pl_trans_kmeans_filter import (
    nonzero_mean,
    spherical_coords,
...
class TestSHGPLTransKmeansFilter(TestCase):
    def setUp(self):
        ...
        self.image_stack = imread(self.test_shg_pl_trans_image_path).mean(axis=-1)
    ...
    def test_binary_classifier_spherical(self):
        centres = np.array([[0.1, 0.3, 0.9], [0.8, 0.3, 0.9], [0.1, 0.5, 0.9]])
        intensities = np.arange(3)
        param = (0.25, 0.5, 1.0, 2.0)

        mask, cost = binary_classifier_spherical(centres, intensities, param)

        np.testing.assert_almost_equal(np.array([1, 0, 0]), mask)
    ...
    def test_cellular_classifier(self):
        ...
        np.testing.assert_almost_equal(np.array([False, True, False]), mask)
    ...
    def test_cluster_colours(self):
        ...
        np.testing.assert_almost_equal(np.arange(10), np.unique(labels))
