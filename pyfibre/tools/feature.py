...
def glcm_difference_props(p_matrix):
    ...
    homogeneity = calculate_metric(p_matrix, 1.0 / (1.0 + square))
    similarity = calculate_metric(p_matrix, 1.0 / (1.0 + absolute))
...
def graycoprops_edit(P):
    ...
    correlation[mask_1] = metrics["covariance"][mask_1] / (
        std_i[mask_1] * std_j[mask_1]
    )
