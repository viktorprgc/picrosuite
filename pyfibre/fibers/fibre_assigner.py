...
from pyfibre.fibers.fibre import fiber_from_node_list
from pyfibre.fibers.fibre_utilities import (
    distance_matrix,
    branch_angles,
    get_node_coord_array,
    get_node_degree_array,
)
...
class FibreAssigner:
    ...
    def _grow_fibre(self, fibre):
        ...
        if n_edges > 0:
            ...
            new_coord_r = np.array([self.r_coord[end_node][n] for n in new_connect])
            ...
            try:
                ...
                fibre.graph.add_node(new_node, xy=self.node_coord[new_node])
                fibre.graph.add_edge(
                    end_node, new_node, r=self.r_coord[new_node][end_node]
                )
    ...
    def assign_fibres(self, graph):
        ...
        for n, node in enumerate(np.argsort(self.edge_count)):
            if tracing[node]:
                ...
                if fibre.graph.number_of_nodes() >= self.min_n:
